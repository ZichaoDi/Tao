...
# mat_contents=sio.loadmat('/homes/wendydi/Documents/Research/APSdata/GlassRod/2dSlice/Slice30.mat')
mat_contents=sio.loadmat('/homes/wendydi/Documents/Research/Tao/XRF_XTM_Simulation/tomopytest.mat')
thetan=mat_contents['thetan1']
...
for fn in range(0,1): # os.listdir('./'):
    XTM = np.array(mat_contents['data_xrf'])
    XTM.shape
    factor=1
    # XTM=np.expand_dims(XTM,axis=2)
    XTM=np.transpose(XTM,(1,0,2))
    # XTM=XTM[:,[2,24,29],:]
    thetan = tomopy.angles(XTM.shape[0],-180,180/factor)
    # rec_XTM.append(tomopy.recon(XTM,thetan,algorithm='mlem',num_iter=10))
    rec_XTM=tomopy.recon(XTM,thetan,algorithm='osem',num_iter=25)

rec_XTM=np.array(rec_XTM)
rec_XTM=np.transpose(rec_XTM,(1,2,0))
rec_XTM.shape
# pylab.imshow(np.reshape(rec_XTM,(rec_XTM.shape[1],rec_XTM.shape[1])),cmap='gray')
slice = np.arange(0,6)
for i in range(0,XTM.shape[1]):
    plt.subplot(1,4,i+1)
    plt.imshow(np.squeeze(rec_XTM[:,:,i]))

plt.show()
# matfile='/homes/wendydi/Documents/Research/APSdata/GlassRod/2dSlice/tomoRec_3_temp.mat'
# sio.savemat(matfile, mdict={'iR': rec_XTM}, oned_as='row')
