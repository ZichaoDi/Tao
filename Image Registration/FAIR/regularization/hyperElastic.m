...
%  }
% 
% see also E0_2DCircleToC_hyperElastic and BigTutorialRegularizer
% ==================================================================================
function [S, dS, d2S] = hyperElastic(uc, omega, m, varargin)
...
if matrixFree
    ...
    if alphaArea~=0 && dim==3,
        ...
        A = A ./ ARef;
        [H,dH,d2H] = phiArea(A,doDerivative);
...
if matrixFree,
    ...
    if alphaArea~=0 || alphaVolume~=0,
        ...
        d2S.diag = @(y) getDiag(omega,m,alphas,y,ARef,VRef);
    else
        d2Snl = @(x,m,yc) 0;
        d2S.diag = @(y) 0;
    end
...
function D = getDiag(omega,m,alphas,y,ARef,VRef)
dim   = length(omega)/2;
...
function [G dG d2G] = phiArea(x,doDerivative)
%
...
[mbS, mbdS, mbd2S] = feval(mfilename,uc,omega,m,'alphaLength',alphaLength,...
    'alphaArea',alphaArea,'alphaVolume',alphaVolume,'matrixFree',0);
[mfS, mfdS, mfd2S] = feval(mfilename,uc,omega,m,'alphaLength',alphaLength,...
    'alphaArea',alphaArea,'alphaVolume',alphaVolume,'matrixFree',1);
