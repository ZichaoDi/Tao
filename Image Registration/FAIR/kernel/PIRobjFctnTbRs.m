...
function [Jc,para,dJ,H] = PIRobjFctnTbRs(T,Rc,omega,omegat,m,mt,beta,M,wRef,xc,xt,wc)
...
xt=reshape(xt,mt(1)*mt(2),2);
% figure(111);plotGrid(xt,omegat,mt);
% pause;hold on; plotGrid(xc,omega,m,'color','r');hold off;
% pause;
overlap=find(omega(1)<=xt(:,1)& xt(:,1)<=omega(2) & omega(3)<=xt(:,2) & xt(:,2)<=omega(4));
indR=find(ismember(reshape(xc,m(1)*m(2),2),xt(overlap,:),'rows'));
[yc,dy] = trafo(wc,xt(:),'doDerivative',doDerivative);
% bdind=[1:m(1),m(1)+1:m(1):(m(1)*m(2)-m(1)),m(1)*m(2)-m(1)+1:m(1)*m(2),2*m(1):m(1):m(1)*m(2)-m(1)];
% Wc=ones(size(Rc));
% Wc(bdind)=0.01;
% Wc=sparse(diag(Wc));
...
dTem=dT*dy;
if(doDerivative & TbRs)
    xcLap=[overlap,size(dT,2)/2+overlap];
    dTem=dTem(overlap,:);
end
[Dc,rc,dD,dr,d2psi] = distance(Tc(overlap),Rc(indR),omega,m,'doDerivative',doDerivative);

% add regularization
% [Sc,dS,d2S] = regularizer(yc-xc,omega,m,'doDerivative',doDerivative);
% if(~isempty(yOld))
% wRef=yOld;
% end
...
para = struct('Tc',Tc,'Rc',Rc,'Rsub',Rc(indR),'omega',omega,'omegat',omegat,'m',m,'mt',mt,'yc',yc,'Jc',Jc);
