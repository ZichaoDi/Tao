...
BG(280:360)=BG(280:360)/10;
load BerylliumTransmission
load simulate_recon195;
b=squeeze(sum(sum(XRF,1),2)).*DetMu;
ind=[9:13,230:360];
BG=squeeze(sum(sum(spectra_30_aligned(1705:end,26:28,:),1),2));
figure,
h1=semilogy(DetChannel,squeeze(sum(sum(spectra_30_aligned,1),2)),'k-','LineWidth',2);
hold on;
h2=semilogy(DetChannel,b*19+1+BG*0*55,'m-','LineWidth',2);
NumElement=8;
cmap=colormap(colorcube(NumElement+1));
...
for ele=1:NumElement
    for el =1:size(BindingEnergy,2)
        if(BindingEnergy(ele,el)>0 & BindingEnergy(ele,el)<maxE)
             ...
             p{ele}=plot([BindingEnergy(ele,el) BindingEnergy(ele,el)],[1 ub],'Color',cmap(ele,:),'LineStyle','--','LineWidth',0.2);
             hold on;
