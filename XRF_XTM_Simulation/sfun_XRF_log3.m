function [f,g]=sfun_XRF_log3(W,xrfData,MU_e,M,NumElement,numChannel,Ltol,GlobalInd,thetan,m,nTau)
    ...
    for i=1:nTau+1
        ...
        if(~isempty(index))
            ...
            for j=1:size(index,1)
                ...
                %% ====================================================================
                temp(1,1,:)=2*bsxfun(@rdivide,reshape(TempSub(:,j,:),NumElement,numChannel),xrfSub)*(log(xrfSub)-log(xrfData{n,i}))';
                g(index(j,2),index(j,1),:)=g(index(j,2),index(j,1),:)+temp;
                clear temp
            end
            sum_Tau=sum_Tau+(log(xrfData{n,i})-log(xrfSub))*(log(xrfData{n,i})-log(xrfSub))';
        end
    end
