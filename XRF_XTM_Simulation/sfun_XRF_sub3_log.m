function [f,g]=sfun_XRF_sub3_log(W,xrfData,MU_e,M,NumElement,numChannel,Ltol,GlobalInd,thetan,m,nTau)
    ...
    for i=1:nTau+1
        ...
        if(~isempty(index))
            ...
            for j=1:size(index,1)
                ...
                %% ====================================================================
                temp(1,1,:,:)=bsxfun(@rdivide,reshape(TempSub(:,j,:),NumElement,numChannel),xrfSub);
%                 figure(111);
%                 for nE=1:NumElement
%                     subplot(NumElement,1,nE)
%                     plot(1:100,reshape(temp(1,1,nE,:),numel(temp(1,1,nE,:)),1),'bo-')
%                     pause;
%                 end
