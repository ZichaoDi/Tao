function [f,g]=sfun_XRF_sub2(W,xrfData,MU_e,M,NumElement,numChannel,Ltol,GlobalInd,thetan,m,nTau)
...
%                                 if(index_after(si,2)==1 & index_after(si,1)==1)
%                                     format long 
%                                     exp(-temp_after)
%                                     MU_e(:,1,tsub+1)
%                                     Lvec_after(si)
                    ...
                    for i_sub=1:i
                        for diffj_i=1:size(SelfInd{index(j,2),index(j,1)}{i_sub}{1},1)
                            ...
                            TempLong=sum(SelfInd{index(j,2),index(j,1)}{i_sub}{2}(diffj_i)*SelfInd{index(j,2),index(j,1)}{i_sub}{4}(diffj_i).*...
                                repmat(SelfInd{index(j,2),index(j,1)}{i_sub}{3}(diffj_i,:)',1,size(M,2)).*M,1);
                            TempSub(:,j,:)=TempSub(:,j,:)-reshape(full(repmat(SelfInd{index(j,2),index(j,1)}{i_sub}{5}(diffj_i,:)',1,size(M,2)).*repmat(TempLong,NumElement,1)),size(TempSub,1),1,size(TempSub,3));
                            clear TempLong
