function [ft,gt]=sfun_full_linear3(W,xrfData,M,NumElement,L,GlobalInd,SelfInd,m,nTau)
...
mtol=prod(m);
W=repmat(reshape(W,[1 mtol NumElement]),[nTau+1 1 1]);
...
for n=1:numThetan
    %%====================================
    temp_v=squeeze(bsxfun(@times,L(n,:,:).*InTens(n,:,:),OutTens(n,:,:,:)));
    TempSub=bsxfun(@times,temp_v,M); % part 3
    XRF_v=sum(sum(bsxfun(@times,temp_v.*W,M),2),3);
    %%======================================        
