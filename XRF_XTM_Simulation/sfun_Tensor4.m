...
for n=1:length(thetan)
    ...
    f=f+sum(SigMa_XRF(count).*sum((XRF_v-cat(1,xrfData{n,:})).^2,2),1);
    g=g+2*reshape(sum(sum(bsxfun(@times,TempSub,repmat(SigMa_XRF(count),[1 1 1 numChannel]).*reshape((XRF_v-cat(1,xrfData{n,:})),nTau+1,1,1,numChannel)),1),4),mtol,NumElement);
end
g=g(:);
