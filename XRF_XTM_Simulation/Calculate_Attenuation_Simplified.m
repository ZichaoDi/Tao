        ...
        if(~isempty(index))
            ...
            for v_count=1:msub
                ...
                if( ~isempty(SelfInd{n,i,v}{2})& ~NoSelfAbsorption)
                    OutTens(n,i,v,:)=exp(-sum(MU(SelfInd{n,i,v}{2},2:NumElement+1),1)*area_xrf(n,i,v)*scaleMU);
                end
...
O2=squeeze(sum(sum(OutTens,1),2));
figure, for ii=1:3, subplot(2,3,ii);imagesc(reshape(MU(:,ii+1),m(1),m(1)));
subplot(2,3,3+ii),imagesc(reshape(O2(:,ii),m(1),m(1)));end
drawnow;
