function [f,g]=sfun_Tensor3_Forward(W,xrfData,M,NumElement,L,GlobalInd,SelfInd,thetan,m,nTau)
                        ...
                        if(~isempty(v7))
                            ...
                            for d=1:NumSSDlet
                                if(~isempty(SelfInd{n,i_sub,v}{7}{d}))
                                    ...
                                    OutTens_J(TempInd,:,:)=OutTens_J(TempInd,:,:)+bsxfun(@times,permute(reshape(OutTens_d(i_sub,SelfInd{n,i_sub,v}{7}{d},d,:),mx,my,mz),[1 2 3]),permute(SelfInd{n,i_sub,v}{8}{d},[3 1 2]));
                                end
                            end
                            OutTens_J=OutTens_J/NumSSDlet;
                            TempSub(i_sub,v,:,:)=TempSub(i_sub,v,:,:)-reshape(squeeze(sum(bsxfun(@times,reshape(L(n,i_sub,v7),length(v7),1).*InTens(i_sub,v7)',bsxfun(@times,W(v7,:),OutTens_J)),1))*M,1,1,NumElement,numChannel);
                        end
                    end
                end
                if(~isempty(v5))
