function [f,g]=sfun_Tensor3(W,xrfData,M,NumElement,L,GlobalInd,SelfInd,thetan,m,nTau)
            ...
            for v=1:mtol
                ...
                if(~isempty(v7))
                    ...
                    for d=1:NumSSDlet
                        if(~isempty(SelfInd{n,i_sub,v}{7}{d}))
                            ...
                            OutTens_J(TempInd,:,:)=OutTens_J(TempInd,:,:)+bsxfun(@times,reshape(OutTens_d(i_sub,SelfInd{n,i_sub,v}{7}{d},d,:)...
                                ,mx,my,mz),permute(SelfInd{n,i_sub,v}{8}{d},[3 2 1]));
                        end
                    end
                    ...
                    TempSub(i_sub,v,:,:)=TempSub(i_sub,v,:,:)-reshape(squeeze(sum(bsxfun(@times,reshape(L(n,i_sub,v7),length(v7),1).*InTens(i_sub,v7)'...
                        ,bsxfun(@times,W(v7,:),OutTens_J)),1))'*M,1,1,NumElement,numChannel); %-- If more than one element, add ' in front of M
                end
            end
        end
    end
