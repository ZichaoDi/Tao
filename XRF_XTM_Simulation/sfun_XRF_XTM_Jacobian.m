function [f,g]=sfun_XRF_XTM_Jacobian(W,xrfData,xtmData,MU_e,M,NumElement,numChannel,Ltol,GlobalInd,LocalInd,L_after,thetan,m,nTau,I0)
    ...
    for i=1:nTau+1
        ...
        if(~isempty(index))
            ...
            if(LogScale)
                ...
                JacobSub2=2*beta*SigMa_XTM(count)*repmat(full(L),[1,1,NumElement]).*repmat(MUe_XTM,[m(1),m(2),1]);
            else
                count=(nTau+1)*(n-1)+i;
                ...
                g=g-2*beta*SigMa_XTM(count)*Rdis*(Rdis-Mt(i)).*repmat(full(L),[1,1,NumElement]).*repmat(MUe_XTM,[m(1),m(2),1]);
                 JacobSub2=-2*beta*SigMa_XTM(count)*Rdis*repmat(full(L),[1,1,NumElement]).*repmat(MUe_XTM,[m(1),m(2),1]);
            end
