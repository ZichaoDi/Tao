...
deltaStar=[delta0';delta_d0]/dTau;
delta=0*deltaStar;
W0=W(:);
x0=[delta;x0];
W0=[deltaStar;W0];
...
%     XTM=aligned(delta,Mt,DetKnot0(1,:),SourceKnot0(1,:),thetan,nTau,numThetan,dTau);
    fctn=@(x)sfun_radonCOR_sim(x,Mt',L_cr(:,:,1));% on attenuation coefficients miu;
    low=[-inf; -inf; -inf;zeros(prod(m)*NumElement,1)];
    up=inf*ones(prod(m)*NumElement+3,1);
    bounds=1;
    if(bounds)
